defmodule EdgeBuilder.Controllers.API.CharacterControllerTest do
  ...
  describe "update" do
    it "updates basic characteristics of a character" do
      ...
      conn = conn() |> authenticate_as(UserFactory.default_user) |> json_put("/api/characters/#{character.permalink}", %{character: %{strain_current: 5}})
    ...
    it "returns errors properly" do
      ...
      conn = conn() |> authenticate_as(UserFactory.default_user) |> json_put("/api/characters/#{character.permalink}", %{character: %{name: ""}})

      assert conn.resp_body |> Poison.decode! == %{"errors" => %{"name" => "can't be blank"}}
    end

    it "requires authentication" do
      conn = conn() |> json_put("/api/characters/doesnt-matter")
    ...
    it "requires the current user to match the owning user" do
      ...
      conn = conn() |> authenticate_as(other) |> json_put("/api/characters/#{character.permalink}", %{character: %{strain_current: 5}})
