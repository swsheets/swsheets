defmodule EdgeBuilder.Repositories.CharacterRepoTest do
  ...
  describe "all" do
    it "returns all characters by page in order" do
      ...
      for c <- characters do
        assert Enum.any?(entries, &(&1.name == c.name)),
               "Expected to find character #{c.name} in #{inspect(entries)}"
      end
    end
  ...
  describe "full_character" do
    ...
    it "preloads all child records" do
      ...
      force_power =
        Repo.insert!(%EdgeBuilder.Models.ForcePower{
          character_id: character_id,
          name: "Force Push"
        })

      Repo.insert!(%EdgeBuilder.Models.ForcePowerUpgrade{
