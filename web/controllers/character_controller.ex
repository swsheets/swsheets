defmodule EdgeBuilder.CharacterController do
  ...
  def create(conn, params = %{"character" => character_params}) do
    ...
    if Changemap.valid?(changemap) do
      ...
      render_new(conn,
        ...
        errors: Ecto.Changeset.traverse_errors(changemap.root, &ErrorHelpers.translate_error/1)
      )
    end
  end
  ...
  def edit(conn, %{"id" => id}) do
    ...
    if !is_owner?(conn, character) do
      ...
      render_edit(conn,
        character: character |> Character.changeset(current_user_id(conn)),
        talents:
          if(Enum.empty?(character.talents), do: [%Talent{}], else: character.talents)
          |> Enum.map(&Talent.changeset/1),
        attacks:
          if(Enum.empty?(character.attacks), do: [%Attack{}], else: character.attacks)
          |> Enum.map(&Attack.changeset/1),
        character_skills: character.character_skills |> CharacterSkill.add_missing_defaults(),
        force_powers:
          if(Enum.empty?(character.force_powers),
            do: [@empty_force_power],
            else: character.force_powers
          )
          |> Enum.map(&to_force_power_changeset/1)
      )
    end
  end
  ...
  def update(conn, params = %{"id" => id, "character" => character_params}) do
    ...
    if !is_owner?(conn, character) do
      ...
      changemap = %{
        ...
        character_skills:
          character_skill_changesets(params["skills"], character.character_skills),
  ...
  defp render_new(conn, assignments \\ []) do
    assignments =
      Keyword.merge(assignments,
        title: "New Character",
        character:
          if(is_nil(assignments[:character]),
            do: %Character{} |> Character.changeset(current_user_id(conn)),
            else: assignments[:character]
          ),
        talents:
          if(is_nil(assignments[:talents]) || Enum.empty?(assignments[:talents]),
            do: [%Talent{} |> Talent.changeset()],
            else: assignments[:talents]
          ),
        attacks:
          if(is_nil(assignments[:attacks]) || Enum.empty?(assignments[:attacks]),
            do: [%Attack{} |> Attack.changeset()],
            else: assignments[:attacks]
          ),
        force_powers:
          if(is_nil(assignments[:force_powers]) || Enum.empty?(assignments[:force_powers]),
            do: [@empty_force_power |> to_force_power_changeset],
            else: assignments[:force_powers]
          ),
        character_skills:
          CharacterSkill.add_missing_defaults(assignments[:character_skills] || [])
      )
  ...
  defp with_upgrades(force_power, upgrade_params) do
    instances =
      case Ecto.Changeset.get_field(force_power, :force_power_upgrades) do
  ...
  defp to_force_power_changeset(force_power) do
    ...
    |> Map.put(
      ...
      Enum.map(force_power.force_power_upgrades, &ForcePowerUpgrade.changeset/1)
    )
