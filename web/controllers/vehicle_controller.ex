defmodule EdgeBuilder.VehicleController do
  ...
  def update(conn, params = %{"id" => id, "vehicle" => vehicle_params}) do
    ...
    changemap = %{
      root: Vehicle.changeset(vehicle, current_user_id(conn), vehicle_params),
      vehicle_attacks:
        child_changesets(params["attacks"], VehicleAttack, vehicle.vehicle_attacks),
      vehicle_attachments:
        child_changesets(params["attachments"], VehicleAttachment, vehicle.vehicle_attachments)
    }
  ...
  defp render_vehicle(conn, :edit, assignments) do
    render_vehicle_base(
      conn,
      :edit,
      Keyword.merge(assignments, title: "Editing #{get_field(assignments[:vehicle], :name)}")
    )
  end
  ...
  defp render_vehicle_base(conn, template, assignments) do
    render(
      ...
      Keyword.merge(assignments,
        ...
        vehicle_attacks:
          if(Enum.empty?(assignments[:vehicle_attacks]),
            do: [%VehicleAttack{} |> VehicleAttack.changeset()],
            else: assignments[:vehicle_attacks]
          ),
        vehicle_attachments:
          if(Enum.empty?(assignments[:vehicle_attachments]),
            do: [%VehicleAttachment{} |> VehicleAttachment.changeset()],
            else: assignments[:vehicle_attachments]
          )
      )
    )
  end
